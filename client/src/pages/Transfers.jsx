...
const Transfers = () => {
  ...
  useEffect(() => {
    ...
    if (filters.start_date) {
      filtered = filtered.filter(p => p.transfer_date >= filters.start_date);
    }
    if (filters.end_date) {
      filtered = filtered.filter(p => p.transfer_date <= filters.end_date);
    }
    if (filters.search) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.asset_type || !formData.quantity || !formData.date || !formData.from_base_id || !formData.to_base_id) {
    ...
    try {
      ...
      setMessage({ type: 'error', text: 'Failed to record transfer' });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      {showForm && (
        <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                    From Base <span className="text-red-500">*</span>
                </label>
                <select
                    name="from_base_id"
            ...
            <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                    To Base <span className="text-red-500">*</span>
                </label>
                <select
                    name="to_base_id"
              ...
              {filteredTransfers.length > 0 ? (
                  ...
                  <tr key={transfer.transfer_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span className="text-sm text-gray-900">{transfer.from_base_name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <Building2 className="h-4 w-4 text-gray-400 mr-2" />
                        <span className="text-sm text-gray-900">{transfer.to_base_name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span className="text-sm text-gray-900">{transfer.created_by_name}</span>
                      </div>
                    </td>
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center">
                    ...
                    <p className="text-gray-400 text-sm">Try adjusting your filters or create a new transfer</p>
                  </td>
                </tr>
              )}
            </tbody>
